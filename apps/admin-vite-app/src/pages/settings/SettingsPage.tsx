...
import { 
  ...
  useBanners, useCreateBanner, useUpdateBanner, useDeleteBanner, useUpdateBannerOrders,
  useSlogans, useCreateSlogan, useUpdateSlogan, useDeleteSlogan, useUpdateSloganOrders,
  useTimelines, useCreateTimeline, useUpdateTimeline, useDeleteTimeline, useUpdateTimelineOrders
} from '@/queries/settings';
import { Loader2, Save, Trash2, GripVertical } from 'lucide-react';
import { RichTextEditor } from '@/components/common/RichTextEditor';
import { SystemSetting, Banner, Slogan, Timeline } from 'shared-api';
...
const SloganCard = ({
  ...
  return (
    <div ref={setNodeRef} style={style} className="group rounded-lg border border-gray-200 bg-white p-4 shadow-sm flex flex-col gap-3">
      ...
      {isEditing ? (
        <div className="space-y-3">
          ...
          <div>
            ...
            <input type="text" value={icon} onChange={e => setIcon(e.target.value)} className={inputCls} placeholder="Biểu tượng..." />
          </div>
          <div>
            <label className="text-[10px] font-bold text-gray-700 uppercase">Mô tả</label>
            <textarea value={description} onChange={e => setDescription(e.target.value)} className={`${inputCls} h-auto py-2`} rows={2} placeholder="Nhập mô tả..." />
        ...
        <div className="group/text cursor-pointer" onClick={() => setIsEditing(true)}>
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-bold text-gray-900 line-clamp-2 hover:text-blue-600 transition-colors" title="Bấm để chỉnh sửa">
              {title || <span className="text-gray-400 italic font-normal">Chưa có tiêu đề</span>}
            </p>
          </div>
...
function SloganSection() {
  ...
  const handleAdd = () => {
    createMutation.mutate({ title: "Slogan mới", icon: "✨", description: "Mô tả slogan", orderIndex: items.length });
  };

  const handleDelete = (id: string) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa slogan này?")) {
      deleteMutation.mutate(id);
    }
  };

  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm space-y-6 mt-6">
      <div className="flex items-center justify-between border-b border-gray-100 pb-3">
        <h2 className="text-sm font-bold text-black">CẤU HÌNH SLOGAN</h2>
        <button type="button" onClick={handleAdd} className="text-xs font-bold bg-black text-white px-3 py-1.5 rounded hover:bg-gray-800">Thêm Slogan</button>
      </div>
      {isLoading ? (
        <div className="flex justify-center py-10"><Loader2 className="h-6 w-6 animate-spin text-gray-400" /></div>
      ) : (
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={items.map(i => i.id)} strategy={rectSortingStrategy}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {items.map((slogan) => (
                <SloganCard 
                  key={slogan.id} slogan={slogan} onDelete={handleDelete}
                  onUpdate={(id, data) => updateMutation.mutate({ id, data })}
                  isDeleting={deleteMutation.isPending}
                />
              ))}
...
const TimelineCard = ({
  ...
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: timeline.id });
  ...
  return (
    <div ref={setNodeRef} style={style} className="group rounded-lg border border-gray-200 bg-white p-4 shadow-sm flex flex-col gap-3">
      <div className="flex justify-between items-start">
        <div {...attributes} {...listeners} className="text-gray-400 hover:text-gray-700 cursor-grab active:cursor-grabbing">
          <GripVertical className="h-5 w-5" />
        </div>
        <button type="button" onClick={() => onDelete(timeline.id)} disabled={isDeleting} className="text-red-400 hover:text-red-600 disabled:opacity-50">
          {isDeleting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
        </button>
      </div>
      {isEditing ? (
        <div className="space-y-3">
          <div><label className="text-[10px] font-bold text-gray-700 uppercase">Năm</label><input type="text" value={year} onChange={e => setYear(e.target.value)} className={inputCls} placeholder="Năm..." /></div>
          <div><label className="text-[10px] font-bold text-gray-700 uppercase">Tiêu đề</label><input type="text" value={title} onChange={e => setTitle(e.target.value)} className={inputCls} placeholder="Tiêu đề..." /></div>
          <div><label className="text-[10px] font-bold text-gray-700 uppercase">Mô tả</label><textarea value={description} onChange={e => setDescription(e.target.value)} className={`${inputCls} h-auto py-2`} rows={3} placeholder="Mô tả..." /></div>
          <div className="flex gap-2 justify-end mt-2">
            <button type="button" onClick={() => setIsEditing(false)} className="px-3 py-1.5 text-xs font-medium text-gray-600 hover:text-black transition-colors">Hủy</button>
            <button type="button" onClick={handleSave} className="px-3 py-1.5 text-xs font-bold text-white bg-black rounded hover:bg-gray-800 transition-colors">Lưu lại</button>
          </div>
        ...
        <div className="group/text cursor-pointer" onClick={() => setIsEditing(true)}>
          <p className="text-blue-600 font-bold">{year || 'Chưa có năm'}</p>
          <p className="text-sm font-bold text-gray-900 mt-1">{title || 'Chưa có tiêu đề'}</p>
          <p className="text-xs text-gray-600 mt-1 line-clamp-3">{description || 'Chưa có mô tả'}</p>
        </div>
      )}
    </div>
  );
...
function TimelineSection() {
  ...
  const handleAdd = () => {
    createMutation.mutate({ year: "2024", title: "Cột mốc mới", description: "Mô tả cột mốc", orderIndex: items.length });
  };
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm space-y-6 mt-6">
      <div className="flex items-center justify-between border-b border-gray-100 pb-3">
        <h2 className="text-sm font-bold text-black">CẤU HÌNH LỊCH SỬ PHÁT TRIỂN</h2>
        <button type="button" onClick={handleAdd} className="text-xs font-bold bg-black text-white px-3 py-1.5 rounded hover:bg-gray-800">Thêm Cột mốc</button>
      </div>
      {isLoading ? (
        <div className="flex justify-center py-10"><Loader2 className="h-6 w-6 animate-spin text-gray-400" /></div>
      ) : (
        <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
          <SortableContext items={items.map(i => i.id)} strategy={rectSortingStrategy}>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {items.map((timeline) => (
                <TimelineCard 
                  key={timeline.id} timeline={timeline} onDelete={handleDelete}
                  onUpdate={(id, data) => updateMutation.mutate({ id, data })}
                  isDeleting={deleteMutation.isPending}
                />
              ))}
