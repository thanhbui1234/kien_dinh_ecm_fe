...
import { Loader2, Trash2, GripVertical, Globe } from 'lucide-react';
import { useSortable } from '@dnd-kit/sortable';
import { CSS } from '@dnd-kit/utilities';
import { Banner } from 'shared-api';
import { Toggle } from '@/components/common/Toggle';
import { inputCls, labelCls } from '@/utils/admin-styles';
import { useSaveBannerTranslation } from '@/queries/settings';
...
export function BannerCard({ banner, enBanner, onToggleStatus, onDelete, onUpdate, isDeleting }: Props) {
  ...
  const [enTitle, setEnTitle] = useState(enBanner?.title ?? '');
  const [enDescription, setEnDescription] = useState(enBanner?.description ?? '');
  const saveTranslation = useSaveBannerTranslation();

  useEffect(() => {
    setEnTitle(enBanner?.title ?? '');
    setEnDescription(enBanner?.description ?? '');
  }, [enBanner]);

  const hasEnContent = !!enBanner?.title;

  const isEnDirty =
    enTitle !== (enBanner?.title ?? '') ||
    enDescription !== (enBanner?.description ?? '');
  ...
  const handleSaveEN = () => {
    saveTranslation.mutate(
      { id: banner.id, data: { lang: 'EN', title: enTitle, description: enDescription } },
  ...
  const handleEnChange = (key: string, value: string) => {
    ...
    else if (key === 'description') setEnDescription(value);
  };
  ...
  return (
    <>
      ...
      >
        <div className="relative aspect-21/9 w-full bg-gray-100 border-b border-gray-200">
          ...
          <button
            ...
            className="absolute bottom-2 right-2 flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold bg-white/90 text-gray-500 hover:bg-purple-100 hover:text-purple-700 transition-colors cursor-pointer opacity-0 group-hover:opacity-100 z-20"
          >
        ...
        <div className="p-4 bg-white flex flex-col gap-3">
          {isEditing ? (
            <div className="space-y-3">
              ...
              <div>
                <label className={labelCls}>Đường dẫn liên kết</label>
                <input type="text" value={link} onChange={(e) => setLink(e.target.value)} className={inputCls} placeholder="Nhập link..." />
              </div>
              <div className="flex gap-2 justify-end mt-2">
                <button type="button" onClick={() => setIsEditing(false)} className="px-3 py-1.5 text-xs font-medium text-gray-600 hover:text-black transition-colors cursor-pointer">Hủy</button>
          ...
          <div className="flex items-center justify-between border-t border-gray-100 pt-3 mt-1">
            ...
            <Toggle checked={banner.status} onToggle={() => onToggleStatus(banner.id, banner.status)} />
          </div>
        </div>
      </div>
      ...
      <EnTranslationModal
        ...
        fields={[
          { key: 'title', label: 'Tiêu đề', viValue: banner.title || '', enValue: enTitle },
          { key: 'description', label: 'Mô tả', viValue: banner.description || '', enValue: enDescription, multiline: true },
        ]}
