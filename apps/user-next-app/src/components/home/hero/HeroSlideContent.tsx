  ...
  return (
    <div className="absolute inset-0 flex items-center px-[clamp(24px,5vw,72px)] max-w-[1440px] mx-auto z-10">
      <div className="w-[min(640px,100%)]">
        <h2
          className={`animate-[hero-rise_0.75s_cubic-bezier(0.22,1,0.36,1)_forwards] opacity-0 translate-y-5 text-[clamp(28px,3.4vw,46px)] font-medium leading-[1.15] tracking-[-0.02em] text-balance ${isDark ? 'text-[#0a0a0a]' : 'text-white'}`}
          style={{ animationDelay: "0.12s" }}
        >
          {title}
          {subtitle && (
            <span
              className="block mt-1.5 opacity-90 text-[clamp(22px,2.5vw,34px)] font-normal leading-[1.2]"
            >
              {subtitle}
            </span>
          )}
        ...
        >
          <div
            className="absolute inset-0 bg-[#ff5901] origin-left scale-x-0 animate-[hero-underline_0.65s_cubic-bezier(0.22,1,0.36,1)_forwards]"
            style={{ animationDelay: "0.48s" }} // Delay 0.24s (from rise) + 0.24s = 0.48s
          />
        </div>

        <p
          className={`text-[15px] font-normal leading-[1.65] max-w-[56ch] m-0 text-pretty opacity-0 translate-y-5 animate-[hero-rise_0.75s_cubic-bezier(0.22,1,0.36,1)_forwards] ${isDark ? 'text-[#0a0a0a]/70' : 'text-white/85'}`}
