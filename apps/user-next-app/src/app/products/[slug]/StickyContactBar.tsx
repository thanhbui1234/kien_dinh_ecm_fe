...
export default function StickyContactBar({
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Hide only when the bottom CTA section enters the viewport
      let isBottomSectionVisible = false;
      if (bottomSectionRef.current) {
        const rect = bottomSectionRef.current.getBoundingClientRect();
        isBottomSectionVisible = rect.top < windowHeight;
      }

      setVisible(!isBottomSectionVisible);
    };

    // Run immediately so bar shows on mount
    handleScroll();
  ...
  return (
    ...
    >
      <div className="bg-white border-t border-gray-100 shadow-[0_-8px_30px_rgba(0,0,0,0.08)] px-6 py-4"
        style={{ paddingBottom: 'max(16px, env(safe-area-inset-bottom))' }}
      >
        <div className="max-w-[1300px] mx-auto flex items-center justify-between gap-4">
          <div className="hidden sm:block min-w-0">
            <p className="text-[12px] font-semibold uppercase tracking-widest text-gray-400 m-0 mb-0.5">
              Sản phẩm
            </p>
