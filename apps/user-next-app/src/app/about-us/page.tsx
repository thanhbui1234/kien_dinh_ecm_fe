...
export default async function AboutUsPage() {
  const [t, profile, companyInfo, historyEvents, facilities, contactSetting, companyLocations] = await Promise.all([
    getTranslations(),
  ...
  return (
    <div className="min-h-screen bg-white pt-[80px]">
      <PageBreadcrumb
        variant="light"
        LinkComponent={Link}
        items={[{ label: t('common.home'), href: '/' }, { label: t('about.breadcrumb') }]}
      />
      ...
      <section className="relative w-full h-[55vh] min-h-[420px] md:h-[72vh] md:min-h-[560px] overflow-hidden bg-[#111]">
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-6 md:px-10 pb-12 md:pb-16 max-w-[1300px] mx-auto left-0 right-0">
          ...
          <h1 className="text-white text-[32px] md:text-[52px] lg:text-[60px] font-light leading-[1.1] tracking-[-0.02em] max-w-[800px] m-0">
            {t('about.hero_brand')}<br />
            <span className="text-white/50">{t('about.hero_title')}</span><br />
            {t('about.hero_location')}
          </h1>
      ...
      {((companyInfo ?? []).length > 0 || introHtml || aboutThumbnail) && (
        <div className="bg-[#fafbfd]">
          <section className="max-w-[1300px] mx-auto px-6 md:px-10 py-20 md:py-28">
            <div className="mb-12">
              <span className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#5e8dd1]">{t('about.section1_eyebrow')}</span>
              <h2 className="text-[36px] md:text-[52px] font-light text-[#111] tracking-[-0.03em] m-0 mt-3 leading-tight">
                {t('about.section1_heading')}
              </h2>
      ...
      {sortedTimelines.length > 0 && (
        <div className="bg-[#eef2f9]">
          <div className="max-w-[1300px] mx-auto px-6 md:px-10 pt-20 md:pt-28 pb-0">
            <span className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#5e8dd1]">{t('about.section2_eyebrow')}</span>
            <h2 className="text-[36px] md:text-[52px] font-light text-[#111] tracking-[-0.03em] m-0 mt-3 leading-tight">
              {t('about.section2_heading')}
            </h2>
            <div className="mt-4 w-12 h-[3px] bg-[#5e8dd1]" />
          </div>
      ...
      {grouped.size > 0 && (
        <section className="max-w-[1300px] mx-auto px-6 md:px-10 py-20 md:py-28">
          <div className="mb-14">
            <span className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#5e8dd1]">{t('about.section3_eyebrow')}</span>
            <h2 className="text-[36px] md:text-[52px] font-light text-[#111] tracking-[-0.03em] m-0 mt-3 leading-tight">
              {t('about.section3_heading')}
            </h2>
            <div className="mt-4 w-12 h-[3px] bg-[#5e8dd1]" />
          </div>
